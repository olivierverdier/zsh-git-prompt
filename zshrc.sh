...
function update_current_git_tag() {
    ## Get the actual tag from git; either an exact match or not
    if [ ${ZSH_GIT_PROMPT_ALWAYS_SHOW_TAG} -eq 1 ]; then
        GIT_CURRENT_TAG=`git describe --tags 2>/dev/null`
    else
        GIT_CURRENT_TAG=`git describe --tags --exact-match 2>/dev/null`
    fi
    ## If we got a tag back, dress it up for placement in the prompt
    if [ ! -z ${GIT_CURRENT_TAG} ]; then
        GIT_TAG_STATUS="${ZSH_THEME_GIT_PROMPT_TAG_PREFIX}${GIT_CURRENT_TAG}${ZSH_THEME_GIT_PROMPT_TAG_SUFFIX}"
    else
        GIT_TAG_STATUS=""
    fi
}
...
git_super_status() {
    ...
    if [ -n "$__CURRENT_GIT_STATUS" ]; then
	  update_current_git_tag
	  STATUS="$ZSH_THEME_GIT_PROMPT_PREFIX$ZSH_THEME_GIT_PROMPT_BRANCH$GIT_BRANCH$GIT_TAG_STATUS%{${reset_color}%}"
...
ZSH_GIT_PROMPT_ALWAYS_SHOW_TAG=0
