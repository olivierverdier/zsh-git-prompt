...
tests :: [TestData]
tests = [
		("## master...ori/master [ahead 3]\n M", "master", [3,0,0,0,1,0,0])
		,
		("## stat\nM ", "stat", [0,0,1,0,0,0,0])
		,
		("## exp...o/exp [ahead 3, behind 2]\n", "exp", [3,2,0,0,0,0,0])
		,
		("## master\nU \nU \nM \nM \nM ", "master", [0,0,3,2,0,0,0])
		,
		("## HEAD (no branch)\n", ":hash", [0,0,0,0,0,0,0])
		,
		("## master\n M\n M\n M\n??\n", "master", [0,0,0,0,3,1,0])
		,
...
makeTest (input, branch, numbers) =
  Just (branch : fmap show numbers) ~=? stringsFromStatus (Just (MkHash "hash")) input (MkStash (last numbers))
